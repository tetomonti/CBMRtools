#This script takes a signature obtained through the OSCC K/D microarray DE analysis and uses ASSIGN to rank TCGA samples based on the same signature 'activitiy'
require(ASSIGN)
require(Biobase)
require(RColorBrewer)

#This function runs ASSIGN given an expression set object (see /restricted/project/montilab-p/projects/oralcancer/scripts/generate_expression_matrix.R for an example of how this was created)

run_assign<-function(ES, #ExpressionSet object 
                     eSig, #Expression signature using which ASSIGN will predict pathway activity (vector of valid gene symbols)
                     oDir, # Output directory where ASSIGN output will be stored (folder is created automatically)
  ...
  mat<-exprs(ES)[fData(ES)$gene_symbol %in% eSig,]
...
fetch_genes <- function(output.data){
...
plotAll<-function(eSet,output.data,title,inclusion_cutoff=0.75,colSideCols=NULL,gene_list){
  ...
  gene_scores<-output.data$mcmc.pos.mean.testData$S_pos[match(rownames(assay.data),names(output.data$processed.data$Pi_matrix))]
  posteriors<-output.data$mcmc.pos.mean.testData$Delta_pos[match(rownames(assay.data),names(output.data$processed.data$Pi_matrix))]
