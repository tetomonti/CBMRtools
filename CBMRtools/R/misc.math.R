...
# it's actually slower than 'dist'
fast.distance <- function( X, Y=NULL, method=c("euclidean","l1") )
{
  if ( is.null(Y) ) {
    Y=X
  }
  if ( nrow(X)!=nrow(Y) ) stop( "nrow(X)!=nrow(Y)" )
  if ( any(rownames(X)!=rownames(Y)) ) stop( "rownames(X)!=rownames(Y)" )

  XY2 <- lapply(1:ncol(Y),function(i) t(X-Y[,i])^2); names(XY2) <- colnames(Y)
  In <- rep(1,nrow(X))
  XY2ss <- sapply(XY2,function(z) sqrt(z %*% In))
  rownames(XY2ss) <- colnames(X)
  XY2ss
}
...
fast.sd <- function( x, cls=NULL, do.sqrt=T, pooled=F, var.equal=T, na.rm=F )
{
