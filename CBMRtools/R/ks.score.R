...
)
{
  ...
  if ( !is.null(weight) )
  {
    ...
    i.max <- which.max(abs(z))
    score <- if (absolute) max(z)-min(z) else z[i.max]
  ...
  if ( !do.pval ) {
      return( c(score=as.numeric(score),index=i.max) )
  }
  ## ELSE compute p-value as in function ks.test but return signed statistic
  ##
  tmp <- suppressWarnings(ks.test(1:n.x,y,alternative=alternative))
  tmp$statistic <- score # use the signed statistic
  return( if (bare) c(score=as.numeric(tmp$statistic), p.value=tmp$p.value, index=i.max) else tmp )
}  
